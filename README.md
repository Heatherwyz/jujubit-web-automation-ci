...
**如何读报告：** 跳过的用例没有验证任何业务行为，因此终端汇总在存在未完成用例时不再显示
“执行通过”，而是显示“执行完成但 N/M 条未完成”，并额外打印**有效覆盖**（得出业务结论的
用例占比）。判断一轮回归是否可信应该看有效覆盖，而不是看通过率——13/30 通过、17 条未完成
的通过率是 100%，但有效覆盖只有 43%。
