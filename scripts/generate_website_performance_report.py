...
#
# 重要：这些是写死在代码里的历史快照，不随每次生成刷新。报告正文必须显式标注
# 采集日期，否则读者会把它们当成本次实测结果。更新数据时同时改 SAMPLE_CAPTURED_AT。
SAMPLE_CAPTURED_AT = "2026-08-19"
PSI_SAMPLES = (
    {
        "page": "首页",
    ...
    *,
    ...
    lines.extend(
        [
            ...
            "",
            f"> **数据来源：{SAMPLE_CAPTURED_AT} 人工采集的历史快照，写死在生成脚本中，"
            "不随本次运行刷新。** 下表数字反映的是当时的站点状态，不是本次实测结果；"
            "需要最新数据请点开外链重新运行 PSI。",
            "",
