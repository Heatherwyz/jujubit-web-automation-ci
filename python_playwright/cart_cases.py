...
CART_CASES = (
    ...
    CartAutomationCase(
        ...
        "分别模拟 cart/add.js 与 cart/change.js 返回 500；change 用真实加号触发，等待数量控件退出加载态后检查页面与服务端购物车。",
        "不出现假加购、非法数量、白屏或 NaN/null 金额；数量、角标、Subtotal、包邮提示和 Checkout 件数恢复到失败前状态，服务端数据不被错误修改。",
    ),
)
