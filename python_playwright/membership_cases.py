...
MEMBERSHIP_CASES = [
    ...
    # MEM-06（低于当前按钮置灰）与 MEM-07（已退订变 Resume）已于 2026-09-23 移除：
    # 需要 Pro 登录态和"已退订但仍在有效期"的账号，长期无法构造，每轮只产出 skip。
    ...
    # MEM-14/15（支付失败兜底与弹窗文案）已于 2026-09-23 移除：需要真实的
    # Airwallex 支付失败，伪造只验证我们自己的 mock。
    # Membership 管理页
    ...
    # MEM-18（付费期内已取消）已于 2026-09-23 移除：需要已取消订阅的账号。
    ...
    # MEM-21（已取消显示 Resume）已于 2026-09-23 移除：需要已取消续费的账号。
    ...
    # 原为"Checkout 入口固定文案"。Checkout 是 Shopify 托管域，注入不到主题
    # 脚本且真实下单有支付副作用；入口契约（entry_page 透传）在购物车侧即可验证。
    MembershipCase("MEM-31", "MEM-31: 购物车入口透传 cart_whole", "test_mem31_checkout_banner_entry_page"),
    ...
    # MEM-36（Premium 超限 toast）已于 2026-09-23 移除：需要 Premium 账号当日
    # 用满 200 次生成，构造成本过高且会真实消耗额度。
    ...
    # MEM-39（支付失败弹窗）已于 2026-09-23 移除：与 MEM-14/15 同因。
    # 埋点
    ...
    MembershipCase("MEM-41B", "MEM-41B: 付费墙曝光只上报一次", "test_mem41b_plan_impression_reported_once"),
